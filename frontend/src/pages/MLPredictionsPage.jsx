...
import { Brain, TrendingUp, Clock, ShieldAlert, Zap } from 'lucide-react';
...
export default function MLPredictionsPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/v1/projects?limit=20`)
      .then(r => r.json())
  ...
  const avgRisk = projects.length ? (projects.reduce((s, p) => s + p.risk_score, 0) / projects.length).toFixed(1) : 0;
  const avgCost = projects.length ? (projects.reduce((s, p) => s + p.cost_overrun_prob, 0) / projects.length * 100).toFixed(1) : 0;
  const avgDelay = projects.length ? (projects.reduce((s, p) => s + p.time_delay_prob, 0) / projects.length * 100).toFixed(1) : 0;
  const highRisk = projects.filter(p => p.risk_score > 75).length;
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h2 className="page-title">ML Risk & Delay Analytics</h2>
            <span style={{
              background: '#eff6ff',
          ...
          <p className="page-subtitle">AI/ML risk scores, cost overrun forecasts & delay estimations across monitored projects</p>
        </div>
      ...
      <div className="ml-grid">
        ...
        <div className="glass-panel dist-panel">
          ...
          <div className="dist-list">
            {[
              { label: 'High Risk (>75)', count: projects.filter(p => p.risk_score > 75).length, color: '#dc2626', bg: '#fef2f2' },
              { label: 'Medium Risk (50–75)', count: projects.filter(p => p.risk_score > 50 && p.risk_score <= 75).length, color: '#d97706', bg: '#fffbeb' },
              { label: 'Low Risk (<50)', count: projects.filter(p => p.risk_score <= 50).length, color: '#16a34a', bg: '#f0fdf4' },
            ].map(d => (
              <div key={d.label} className="dist-item" style={{ background: d.bg }}>
                ...
                <div className="dist-bar-bg">
                  <div className="dist-bar-fill" style={{ width: `${projects.length ? (d.count / projects.length) * 100 : 0}%`, background: d.color }}></div>
                </div>
                <div className="dist-pct" style={{ color: d.color }}>
                  {projects.length ? Math.round((d.count / projects.length) * 100) : 0}%
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="glass-panel table-wrap">
        <div className="panel-h-row">
          ...
          <span className="pred-note">Updated daily via automated model pipeline</span>
        </div>
          ...
          <tbody>
            {loading
              ? Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i}><td colSpan={8}><div className="row-skel"></div></td></tr>
                ))
